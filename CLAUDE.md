...
- **`excalidraw-app/components/SettingsDialog.tsx`** contains the native app's General, Sync, Shortcuts, and About settings. Shortcut overrides persist locally and the app shell can open it through the native menu; `App.tsx` tracks which section to open (`settingsSection`) so the rail's sync badge can land on Sync directly.
- **Page palette vs. new page.** `⌘T` opens **`excalidraw-app/components/PagePalette.tsx`** — a search-first "open a page" surface (ranking in `data/pagePalette.ts`, recency in `data/recentPages.ts`, which is localStorage-only so it never dirties `workspace.json`). `⌘N` opens **`NewPageDialog.tsx`**, which creates a page and therefore asks for folder → file → name. Keep those two responsibilities separate; the tab strip's "+" is an _open_, not a _create_.
...
### GitHub sync (optional backup transport)

- **`electron/githubSync.ts`** is the engine: `isomorphic-git` over the workspace directory, no `git` CLI dependency, no `electron` import (dependency-injected so it is testable against a temp dir). Exports pure helpers (`parseGithubRepository`, `millisecondsUntilNextSync`, `describeSyncSummary`, `countWorkingTreeChanges`) plus `createGithubSyncEngine`.
- Sync order is: stage everything → commit if dirty → fetch → merge → push. It **never guesses** when local and remote have both moved: it reports a `conflict` state and waits for the user to choose `keep-local`/`keep-remote`, and it copies the workspace to a dated backup folder before any operation that overwrites local files.
- **`electron/settingsStore.ts`**'s `createSettingsGithubSyncStore` persists repo config in `userData/settings.json`; the access token is encrypted through Electron `safeStorage` (injected from `main.ts`) and is never returned to the renderer — the bridge only ever exposes `hasToken`.
- **`electron/main.ts`** owns the IPC surface (`github-sync:*`), the auto-sync timer (clamped to a 60s minimum), and the optional sync-on-launch.
- Renderer side: **`excalidraw-app/data/githubSyncTypes.ts`** (shared, DOM-free — also listed in `electron/tsconfig.json`), **`data/githubSync.ts`** (transport + `useGithubSyncStatus`), **`components/GithubSyncSettings.tsx`** (Settings → Sync), and the `SyncRailButton` in `LeftDock.tsx`. There is no browser fallback — git needs a filesystem — so outside Electron every call reports `isSupported: false` and the UI says so.
